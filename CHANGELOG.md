...
- Two project-scope browser subagents (`qa-page-analyzer`, `qa-test-runner`), each spawning its own Playwright MCP process; installed into `.claude/agents/` by `/qa-catalog:init`.
...
- 18 `userConfig` knobs (dev URL, parallelism, browser channel, headless, settle ms, auth, roles, task depth, exclude globs).
