...
| `qa-my-app:test-author` | plugin | Pure markdown — converts page analysis into task files using the enforced template. Reads the route source directly for exact validation rules; happy paths fill every field and **submit for real**. Accepts ticket acceptance criteria and emits an `## Acceptance criteria` block. | `Read, Write, Edit, Glob` |
