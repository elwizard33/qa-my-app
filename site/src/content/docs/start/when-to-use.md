...
7. **Verify just what changed** with `/qa-catalog:verify` — scopes from the conversation + uncommitted diff, a branch/PR range, or a connected ticket's acceptance criteria, then re-authors and runs only the affected tasks.
