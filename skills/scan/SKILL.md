...
allowed-tools: Read, Grep, Glob, Write, Edit, Bash(node *), Bash(git *), Bash(mkdir *), Agent(qa-my-app:route-discoverer), Agent(qa-page-analyzer), Agent(qa-my-app:test-author)
---
...
Use `$ARGUMENTS` as the dev URL override if provided; otherwise resolve as `/qa-my-app:init` does.

After the rescan, print the same summary as `/qa-my-app:init` plus the backup location.
