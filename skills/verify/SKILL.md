...
allowed-tools: Read, Grep, Glob, Write, Edit, AskUserQuestion, Bash(node *), Bash(git *), Bash(mkdir *), Bash(start *), Bash(xdg-open *), Bash(open *), Agent(qa-page-analyzer), Agent(qa-catalog:test-author), Agent(qa-test-runner)
---

# /qa-catalog:verify — Test what changed (or what a ticket asks for)
...
- **An issue/ticket id** (matches `[A-Z]+-\d+` like `PROJ-123`, or a numeric `#123`) → treat as a ticket:
  ...
  3. If no tracker MCP is connected, tell the user how to connect one (`/qa-catalog:init` Phase 1 prints the commands) and fall back to change-scope for this run.
...
If scope resolution yields **no routes**, say so plainly and stop (offer `/qa-catalog:run-all` for a full run). If `change-scope.mjs` reports `unmatched` files that look like UI source, mention them — they may be new routes the catalog hasn't seen, in which case suggest `/qa-catalog:sync` first.
...
1. Spawn `qa-page-analyzer` (one per route, batched up to `${user_config.parallel_agents}`) with the standard analysis payload (same shape as `/qa-catalog:init` Phase 3, secrets resolved via `node "${CLAUDE_PLUGIN_ROOT}/scripts/auth-resolve.mjs" --json` when `auth_mode` is `per-role`).
2. Spawn `qa-catalog:test-author` for each analysis, passing the normal `settings` PLUS:
   ```json
   {
...
Reuse the `/qa-catalog:run-all` dispatch/verify loop, scoped to just the collected task ids:
...
Write `<runRoot>/summary.md` as `/qa-catalog:run-all` does, then append history + render the final dashboard:
```bash
